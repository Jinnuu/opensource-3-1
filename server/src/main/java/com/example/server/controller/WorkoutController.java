...
public class WorkoutController {
    ...
    public ResponseEntity<?> getWorkouts(@RequestParam(required = true) String userName) {
        try {
            ...
            // Workout을 WorkoutData로 변환
            List<WorkoutData> workoutDataList = workouts.stream()
                .map(workout -> {
                    WorkoutData data = new WorkoutData();
                    data.setExerciseType(workout.getExerciseType());
                    data.setDuration(workout.getDuration());
                    data.setCompleted(workout.isCompleted());
                    data.setRoutineName(workout.getRoutineName());
                    data.setExerciseNumber(workout.getExerciseNumber());
                    data.setTotalExercises(workout.getTotalExercises());
                    // Date를 LocalDateTime으로 변환
                    data.setWorkoutDate(java.util.Date.from(
                        workout.getDate().atZone(java.time.ZoneId.systemDefault()).toInstant()
                    ));
                    return data;
                })
                .collect(java.util.stream.Collectors.toList());
            
            return ResponseEntity.ok(workoutDataList);
        } catch (Exception e) {
            e.printStackTrace();
